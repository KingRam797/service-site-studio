...
function CommitHero() {
  return (
    <div className="commit-stage" aria-label="A project moving from first commit to live launch">
      <div className="commit-window">
        <div className="window-bar"><span /><span /><span /><code>push2start / production</code></div>
        <div className="commit-body">
          <div className="commit-line is-done"><i /><div><code>commit 01</code><strong>business mapped</strong><small>offer · proof · customer path</small></div></div>
          <div className="commit-line is-done"><i /><div><code>commit 02</code><strong>direction approved</strong><small>identity · primary page · interaction</small></div></div>
          <div className="commit-line is-live"><i /><div><code>push origin main</code><strong>READY_TO_LAUNCH</strong><small>verified · transferred · live</small></div></div>
        </div>
        <div className="terminal-line"><span>$</span><code>git push business live</code><b aria-hidden="true" /></div>
      </div>
      <div className="launch-stamp"><BranchMark /><span>2 weeks<br />or less</span></div>
    </div>
  );
}
            ...
            {config.proof.items.map((item, index) => (
              <article className="proof-card" key={item.title}>
                ...
                <div className="project-copy">
                  <p>{item.category}</p><h3>{item.title}</h3><span>{item.description}</span>{item.result && <strong>{item.result}<b>↗</b></strong>}
                </div>
              </article>
            ))}
          </div>
